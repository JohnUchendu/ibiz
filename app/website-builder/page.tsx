    ...
    <section className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Pick a plan that suits your stage — all plans are built with speed & SEO in mind.
          </p>
        </div>

        <div className="flex items-center gap-3 bg-muted/50 p-1 rounded-full">
          <span className={`px-3 py-1 rounded-full cursor-pointer ${billing === "monthly" ? "bg-white shadow" : ""}`} onClick={() => setBilling("monthly")}>
            Monthly
          </span>
          <span className={`px-3 py-1 rounded-full cursor-pointer ${billing === "yearly" ? "bg-white shadow" : ""}`} onClick={() => setBilling("yearly")}>
            Yearly
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <article key={plan.id} className={`relative rounded-2xl p-6 border ${isPremium ? "bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 text-white shadow-2xl scale-105" : "bg-white"}`}>
              {isPremium && (
                <div className="absolute -top-3 right-4 bg-amber-500 text-slate-900 px-3 py-1 text-xs rounded-xl font-semibold">Premium</div>
              )}

              <header className="mb-4">
                <h3 className={`text-xl font-bold ${isPremium ? "text-white" : "text-slate-900"}`}>{plan.title}</h3>
                <p className={`text-sm mt-1 ${isPremium ? "text-amber-100" : "text-slate-600"}`}>{plan.description}</p>
              </header>

              <div className="mb-6 flex items-baseline gap-3">
                <span className={`text-3xl font-extrabold ${isPremium ? "text-white" : "text-slate-900"}`}>₦{price.toLocaleString()}</span>
                <span className={`text-sm ${isPremium ? "text-amber-100" : "text-slate-500"}`}>/ {billing === "monthly" ? "month" : "year"}</span>
              </div>

              <ul className="mb-6 space-y-3">
                {plan.features.map((f) => (
                  <li key={f} className={`text-sm leading-snug ${isPremium ? "text-amber-50" : "text-slate-700"}`}>• {f}</li>
                ))}
              </ul>

              <div className="mt-auto">
                <a href={plan.cta} className={`inline-block w-full text-center font-semibold py-3 rounded-lg ${isPremium ? "bg-amber-400 text-slate-900 hover:bg-amber-300" : "bg-slate-900 text-white hover:opacity-95"}`} data-plan-id={plan.id}>
                  Get yours
                </a>
      ...
      <section id="contact" className="px-6 py-16 bg-gray-100 text-center">
        ...
        <p className="text-gray-600 mb-6">Ready for your free website? Reach out to our support team now.</p>
        <a href="mailto:support@yourdomain.com">
          <Button size="lg" className="bg-blue-600 hover:bg-blue-700">Email Us</Button>
        </a>
      </section>
